...
#KubepkgDashboard: kubepkg.#KubePkg & {
	metadata: name: string | *"kubepkg-dashboard"
	spec: {
		version: _
		...
		config: KUBEPKG_METRIC_ENABLE_SIMPLE_AGGREGATION: string | *"false"
		config: KUBEPKG_METRIC_COLLECTOR_ENDPOINT:        string | *""
		config: KUBEPKG_METRIC_COLLECT_INTERVAL_SECONDS:  string | *"0"
		...
		config: KUBEPKG_PUBLIC_ENDPOINT:                  string | *""

		services: "#": ports: containers."kubepkg-dashboard".ports
