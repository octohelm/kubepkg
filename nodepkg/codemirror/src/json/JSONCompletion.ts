...
import { get, has, last, map } from "@nodepkg/runtime/lodash";
  ...
  if (node) {
    switch (node.name) {
      case "Object":
        ...
        for (
          let p: SyntaxNode | null = node.firstChild;
          p != null;
          p = p.nextSibling
        ) {
          if (p.name === "Property") {
            ...
            if (propName && p.lastChild) {
              obj[unquote(state.sliceDoc(propName.from, propName.to))] =
                astNodeToValue(state, p.lastChild);
            }
          }
        }
        return obj;
    ...
    if (node.parent) {
      ...
      switch (parent.name) {
          ...
          if (selectionSet.node.name !== "PropertyName") {
            ...
            if (prop) {
              selectionSet.instancePath.push([
                prop,
                astNodeToValue(state, parent.parent),
              ]);
            }
          }
          return;
        case "Array":
          let idx = 0;
          for (
            let item: SyntaxNode | null = parent.firstChild;
            item != null;
...
export class JSONCompletion {
  ...
  private _resolveCompletions(
    ...
    if (
      selection.node.name === "Object" ||
      selection.node.name === "PropertyName"
    ) {
      ...
      if (schema instanceof SchemaUnionType) {
        if (schema.discriminator) {
          ...
          const discriminatorValue = get(obj || {}, [
            prop,
          ...
          if (!discriminatorValue) {
            return [
              snippetCompletion(`"${schema.discriminator}": "\${}"`, {
      ...
      for (const schema of list) {
        for (const propName of schema.propNames) {
          ...
          if (selection.node.name == "Object") {
            ...
            if (
              subSchema instanceof SchemaUnionType &&
              subSchema.discriminator
            ) {
              snippetHolder = "{${}}";
