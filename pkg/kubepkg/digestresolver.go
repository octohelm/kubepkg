...
func (r *DigestResolver) Resolve(ctx context.Context, pkg *v1alpha1.KubePkg) (*v1alpha1.KubePkg, error) {
	...
	for name := range p.Spec.Images {
		...
		if err := r.resolve(ctx, repo, imgCtx, &p.Status); err != nil {
			return nil, err
		}
	}
...
func (r *DigestResolver) resolve(ctx context.Context, repo distribution.Repository, imgCtx *imageCtx, status *v1alpha1.KubePkgStatus) error {
		...
		case manifestlist.MediaTypeManifestList:
			for _, sub := range m.References() {
				...
				if subImgCtx.isNeededPlatform(subImgCtx.platform) {
					if err := r.resolve(ctx, repo, subImgCtx, status); err != nil {
						return err
					}
				}
			}
	...
	if err := r.resolve(ctx, repo, imgCtx.withDigest(d.Digest), status); err != nil {
		return err
	}
