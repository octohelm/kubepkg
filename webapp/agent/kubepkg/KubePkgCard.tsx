import { KubeManifest, KubePkg } from "../../client/agent";
...
const flattenKubePkg = (kubepkg: KubePkg): { [k: string]: KubeManifest } => {
  const flattenManifests: { [k: string]: KubeManifest } = {};

  const walk = (manifests: { [k: string]: KubeManifest | { [k: string]: KubeManifest } }, parent = "") => {
...
export const KubePkgManifestStatus = ({ manifest }: { manifest: KubeManifest }) => {
    ...
    return (
      <Stack component="span" flexDirection={"row"} gap={0.5}>
        {map(manifest.status.conditions, (c, i) => {
          const color = c.status === "True" ? theme.palette.success : theme.palette.error;
  ...
  return (
    <Box>
      ...
      <Typography color="text.primary">
        <Box component="span" sx={{ display: "flex", alignItems: "center" }} onClick={() => detailOpen$.next(true)}>
        ...
        <Subscribe value$={detailOpen$}>
          {(open) => {
            return (
              ...
              >
                <DialogTitle sx={{ display: "flex" }}>{manifest.metadata.name}</DialogTitle>
                <DialogContent>
...
export const KubePkgCard = ({ kubepkg }: { kubepkg: KubePkg }) => {
