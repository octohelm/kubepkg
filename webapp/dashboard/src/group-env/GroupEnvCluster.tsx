...
export const useGroupEnvClusterBindDialog = () => {
  ...
  const dialog$ = useDialog({
    ...
    content: (
      <ClusterMenuList
        onSelect={(clusterID) => groupEnv$.bindCluster$.next({
            ...
            return (
              <Stack direction={"row"} spacing={1} sx={{ alignItems: "center" }}>
                ...
                <AccessControl op={groupEnv$.bindCluster$}>
                  <IconButtonWithTooltip label="关联集群" onClick={() => bindDialog$.next(true)}>
                    <Link />
          ...
          return (
            <Stack direction={"row"} spacing={1} sx={{ alignItems: "center" }}>
              ...
              <AccessControl op={groupEnv$.unbindCluster$}>
                <IconButtonWithTooltip label="解绑集群" onClick={() => unbindDialog$.next(true)}>
                  <LinkOff />
                </IconButtonWithTooltip>
                {unbindDialog$.render()}
              </AccessControl>
