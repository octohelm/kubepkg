...
import { List, ListItem, ListItemText, Box, Link, Tooltip, Stack, Theme, useTheme } from "@mui/material";
import {
  kubepkgName,
  deploymentID,
  deploymentSettingID,
...
const InfoSpan = ({ label, children }: { label?: ReactNode, children: ReactNode }) => {
  return (
    <Box
      component={"span"}
      sx={{
...
const DeploymentEndpoints = ({ kubepkg }: { kubepkg: ApisKubepkgV1Alpha1KubePkg }) => {
              ...
              >
                ...
                {openAPISpec && (
                  <Tooltip
                    title={"OpenAPI 文档"}
                  >
                    <Link target={"_blank"} href={openAPISpecDoc(`${address}${openAPISpec}`)}>
                      {openAPISpec}
                    </Link>
                  </Tooltip>
                )}
...
const ReasonMessage = ({ reason, message }: { message: string, reason: string }) => {
...
const ContainerStatuses = ({ name, kubepkg }: { name: string, kubepkg: ApisKubepkgV1Alpha1KubePkg }) => {
  return (
    <Box component={"span"}>
      {map(kubepkg.status?.resources, (r: any, i) => {
        if (r.apiVersion == "apps/v1") {
          ...
          return (
            <Fragment key={i}>
              {map(latestPodStatus?.containerStatuses, (containerStatus: any, i) => {
...
export const SpecStatus = ({ kubepkg }: { kubepkg: ApisKubepkgV1Alpha1KubePkg }) => {
  return (
    <Box component={"span"}>
      <InfoSpan>
        {map(kubepkg.status?.resources, (r: any, i) => (
...
const DeploymentConditions = ({ kubepkg }: { kubepkg: ApisKubepkgV1Alpha1KubePkg }) => (
  <InfoSpan>
    <Stack spacing={0.5} sx={{ alignItems: "flex-end" }}>
      <Stack component={"span"} direction={"row"} spacing={0.5}>
        {map(orderBy(kubepkg.status?.resources, (r: any) => r.apiVersion), (r: any, i) => {
          return (
            <Tooltip title={`${r.metadata.name}.${r.kind}/${r.apiVersion}`} key={i}>
              <Box component={"span"}>
                <ConditionBox
                  type={r.kind}
                  status={statusOf(r)}
                ...
                <Stack component={"span"} spacing={0.5} direction={"row"}>
                  <Stack component={"span"} spacing={0.5}>
                    {map(status.podStatuses, (podStatus, i) => (
                      <Stack key={i} component={"span"} direction={"row"} spacing={0.5}>
                        {map(podStatus.conditions, (c, i) => (
                          <Tooltip title={`${c.type}${c.message ? `: ${c.message}` : ""}`} key={i}>
                            <Box component={"span"}>
                              <ConditionBox
                                type={c.type}
                                status={c.status}
...
const KubePkgHeading = ({ kubepkg }: { kubepkg: ApisKubepkgV1Alpha1KubePkg }) => {
  const theme = useTheme();
        ...
        {(kubepkg as any)?.upgrade && (
          <Box
            ...
            sx={{
              ...
              paddingLeft: 0.5,
              paddingRight: 0.5,
              borderColor: (kubepkg as any)?.upgrade?.latest ? theme.palette.success.light : theme.palette.warning.light,
              color: (kubepkg as any)?.upgrade?.latest ? theme.palette.success.light : theme.palette.warning.light
            }}
          >
            {(kubepkg as any)?.upgrade?.latest ? "最新" : "有更新"}
          </Box>
        )}
      </Stack>
    </Box>
      ...
      <ListItemText
        primary={
          <Stack spacing={1} component={"span"} direction={"row"}>
            <Stack spacing={1} component={"span"} direction={"row"} sx={{ alignItems: "center", "flex": 1 }}>
...
export const GroupEnvDeploymentExport = () => {
  ...
  return (
    <IconButtonWithTooltip
      label={"导出配置"}
      onClick={() => {
        toKubePkgList(`${groupEnvDeployments$.envName}.${groupEnvDeployments$.groupName}`, groupEnvDeployments$.value);
      }}
    >
      <Download />
    </IconButtonWithTooltip>
...
export const GroupEnvDeploymentList = () => {
  ...
  const deployments = useMemo(() => orderBy(values(groupEnvDeployments || {}), (d) => d.spec.deploy?.kind), [groupEnvDeployments]);
