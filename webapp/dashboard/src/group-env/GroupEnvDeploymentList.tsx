...
import { deploymentID, deploymentSettingID, GroupEnvDeploymentsProvider, openAPISpecDoc, revision } from "../group";
...
const KubePkgHeading = ({ kubepkg }: { kubepkg: ApisKubepkgV1Alpha1KubePkg }) => {
  return (
    ...
    >
      ...
      >
        {kubepkg.spec.deploy?.kind}/{kubepkg.metadata?.name}@{kubepkg.spec.version}
      </Box>
    </Box>
  );
};
      ...
      <ListItemText
        primary={
          <Stack spacing={1} component={"span"} direction={"row"}>
            <Stack spacing={1} component={"span"} direction={"row"} sx={{ "flex": 1 }}>
