...
export const GroupEnvDeploymentsProvider = createDomain(({}, use) => {
  ...
  const groupEnvDeployments$ = use(
    ...
    (groupEnvDeployments$) =>
      groupEnvDeployments$.list$.pipe(rxMap((resp) =>
        reduce(
          resp.body.data,
          (ret, kpkg) => ({
            ...ret,
    ...
    (groupEnvDeployments$) =>
      merge(
        groupEnvDeployments$.list$,
        groupEnvDeployments$.put$
      ).pipe(
        tap(() => {
          const kubepkgNames = map(
            groupEnvDeployments$.value,
            (kpkg) =>
              `${kubepkgName(kpkg)}@${channel(kpkg) || KubepkgChannel.DEV}`
          );
  ...
  useEffect(() => {
    const fetch = () => groupEnvDeployments$.list$.next({
      groupName: groupEnvDeployments$.groupName,
      envName: groupEnvDeployments$.envName,
      size: -1
    });
