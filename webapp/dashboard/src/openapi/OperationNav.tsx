...
const GroupedOperationList = ({ group, operations }: { group: string; operations: any[] }) => {
          ...
          <ListItemText
            primary={
              <StyledOperationDesc deprecated={operation.deprecated}>
                ...
                <Box sx={{ fontSize: "0.6em", opacity: 0.7 }}>{operation.summary}</Box>
              </StyledOperationDesc>
            }
          />
        </ListItem>
...
export const OperationNav = () => {
  ...
  const groupedOperations = useMemo(() => {
    const finalOperations = filterValue
      ? filter(operations, (k) => String(k.operationId || "").includes(filterValue))
      : operations;
      ...
      >
        <List>
          {map(groupedOperations, (operations: any[], group: string) => (
            <GroupedOperationList key={group} group={group} operations={operations} />
          ))}
        </List>
      </Box>
    </Stack>
