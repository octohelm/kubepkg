...
export const RequestBuilder = ({ operation }: { operation: any }) => {
  ...
  const [requestSchema, toRequest] = useMemo(() => {
    ...
    const pickNamesBy = (by: (p: any) => boolean) => map(
    ...
    return [
      {
        ...
        properties: reduce(parameters, (props, p) => {
          return {
            ...props,
            [p.name]: {
              ...p.schema,
      ...
      (inputs: any): RequestConfig<any> => {
        const c: RequestConfig<any> = ({
          ...
          url: `${operation.basePath || ""}${compilePath(operation.path, pickValuesIn("path", parameters, inputs))}`,
  ...
  return (
    <Stack spacing={1}>
      <Stack direction={"row"} spacing={5}>
        <Stack sx={{ flex: 1, maxWidth: "50%" }} spacing={1}>
          <Editor
            height="400px"
            theme="vs-dark"
            language="json"
            ...
            beforeMount={(monaco) => {
              const schemas = map({
                ...
                return {
                  ...
                  fileMatch: k == "RequestParameter" ? ["*.RequestParameter.json"] : []
                };
              });
      ...
      <Subscribe value$={resp$}>
        {(resp) => resp.status ? (
          <HTTPResponse response={resp as any} />
        ) : null}
      </Subscribe>
    </Stack>
  );
