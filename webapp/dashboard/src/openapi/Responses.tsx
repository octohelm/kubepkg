...
const dropHttpError = (s = ""): string =>
  replace(s, new RegExp(reStatusError, "g"), "");

const HttpErrorList = ({
  httpErrorList,
}: {
  httpErrorList: IStatusError[];
}): JSX.Element => (
  <Box>
  ...
  return (
    <>
      <Stack direction={"row"} sx={{ py: 2 }} spacing={5}>
        <Box sx={{ width: "30vw", display: "flex", alignItems: "flex-start" }}>
          <Box>
            <HttpCode code={code} />
          </Box>
          <Box sx={{ flex: 1, ml: 1 }}>
