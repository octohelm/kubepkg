...
export const toEnumMap = (schema: any) => {
  ...
  forEach(schema.enum, (value, i) => {
    enumMap[value] = get(schema, ["x-enum-options", i, "label"], get(schema, ["x-enum-labels", i], value));
  });

  return enumMap;
};

export const enumToOptions = <T extends any>(list: T[], enumMap: { [key: string]: string }): Array<IOption<T>> =>
  map(list, (value) => ({
    label: `${String((enumMap as any)[value]) || " "} ${String(value)}`,
    value
  }));

const Type = ({ type, format, id }: { type: string, format: string, id?: string }) => {
  ...
  if (type === "circular" && id) {
    return (
      <a href={`${location.pathname}${location.search}#~${id}`}>
        <Box component={"small"} sx={{ opacity: 0.6, fontWeight: "normal" }}>{`~${id}`}</Box>
      </a>
    );
  }
  ...
  return (
    <Box
      component={"span"} id={id ? `~${id}` : undefined}
      sx={id && location.hash === `#~${id}` ? {
...
export const displayType = (schema: any, id?: string) => {
  if (schema.type === "array" && schema.items) {
    return (
      <span>
        <span>{`[${String(schema.minItems && schema.minItems === schema.maxItems ? schema.minItems : "")}]`}</span>
...
export const displayDefault = (schema: any): string => {
  ...
  const defaultValue: string = schema.type === "string" ? JSON.stringify(schema.default) : schema.default;
...
const EnumLabel = ({ title, children }: { title: string; children: string }) => (
  <Box sx={{ opacity: 0.6, display: "flex", lineHeight: 1.4, fontSize: 10 }}>
    <span>{title}</span>
    <Box component={"span"} sx={{ fontWeight: "normal", opacity: 0.8, marginLeft: "1em" }}>{`// ${children}`}</Box>
  </Box>
);

export const displayClassName = (schema: any): JSX.Element => {
  if (schema.enum) {
    return (
...
interface ISchemaRowProps extends ISchemaProps {
  ...
  typeRenderer?: (type: string | JSX.Element, schema: any) => ReactNode;
}
...
const StyledSchemaName = ({
                            ...
                            nullable,
                            ...otherProps
                          }: { deprecated?: boolean; nullable?: boolean; required?: boolean } & React.HTMLAttributes<any>) => {
  const theme = useTheme();
  ...
  return (
    <Box
      ...
      sx={[
        ...
        nullable ? { "&:after": { content: `"??"`, color: theme.palette.error.main } } : {},
        !required ? { "&:after": { content: `"?"`, color: theme.palette.warning.main } } : {}
      ]}
    />
  );
};
...
const defaultNameRenderer = (name: string, required: boolean, schema: any = {}) =>
  name && (
    <StyledSchemaName required={required} deprecated={schema.deprecated} nullable={schema.nullable}>
      {name}
    </StyledSchemaName>
  );

const defaultTypeRenderer = (type: any) => <StyledSchemaType>{type}</StyledSchemaType>;

export const Description = ({ desc, prefix }: { desc: string; prefix?: string }) => {
...
const SchemaRow = ({
  ...
  const description = `${schema["x-param-in"] ? `@in ${schema["x-param-in"]}${schema["x-content-type"] ? ` Content-Type = ${schema["x-content-type"]}` : ""} ` : ""}${schema.description || ""}`;
...
const renderSchema = (schema: any, name?: string, required?: boolean): JSX.Element => {
          ...
          return (
            <Box sx={asMap ? { display: "flex" } : {}}>
              {asMap ? (
                ...
                <StyledSchemaType>
                  {type}
                  <Box component={"span"} sx={{ opacity: 0.5, fontWeight: "normal" }}>{"{}"}</Box>
                </StyledSchemaType>
              )}
              <div>
                {asMap ? (
                  ...
                  <>
                    {map(get(schema, "x-id") === "RequestParameter" ? keys(schema.properties || {}) : sortBy(keys(schema.properties || {})), (propName: string) => {
...
export const SchemaView = ({ schema, ...otherProps }: ISchemaProps) => {
  ...
  return (
    <Box {...otherProps} sx={{ fontSize: "14px" }}>
