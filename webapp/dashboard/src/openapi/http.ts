...
const pickKeysIn =
  (where: string) =>
    (parameters: any[]): string[] =>
      map(pickParametersIn(where)(parameters), (parameter: any) => parameter.name);
...
export const pickValuesIn = (where: string, parameters: any[], values: any) => ({
...
export const compilePath = (path: string, params: Dictionary<any> = {}): string =>
...
export const getContentType = (headers: any = {}): string => headers["Content-Type"] || headers["content-type"] || "";

export const isContentTypeMultipartFormData = (headers: any) => getContentType(headers).includes("multipart/form-data");
export const isContentTypeJSON = (headers: any) => getContentType(headers).includes("application/json");
export const isContentTypeFormURLEncoded = (headers: any) =>
  getContentType(headers).includes("application/x-www-form-urlencoded");
export const isContentTypeTextPlain = (headers: any) => getContentType(headers).includes("text/plain");
export const isContentTypeTextHTML = (headers: any) => getContentType(headers).includes("text/html");
